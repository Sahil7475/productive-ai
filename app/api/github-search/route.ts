...
/* function mergeResults(repos: any[], codeMatches: any[]): any[] {
  // Map code matches by repo full_name
  const codeByRepo: Record<string, any[]> = {}
  for (const match of codeMatches) {
    const repoName = match.repository.full_name
    if (!codeByRepo[repoName]) codeByRepo[repoName] = []
    codeByRepo[repoName].push({
      file_name: match.name,
      file_url: match.html_url,
      snippet: match.text_matches?.[0]?.fragment || "",
    })
  }
  // Merge code matches into repos
  return repos.map((repo) => {
    ...
    return {
      ...
      code_matches: codeByRepo[fullName] || [],
    }
  })
} */
